...
# 1. 単体攻撃
execute as @s[scores={job_id=3}] at @s run effect give @s haste 30 1 false
...
# 2. コンボ
execute as @s[scores={job_id=3}] at @s run item replace entity @s hotbar.1 with red_dye[max_stack_size=1,custom_data={"combinat_data":{"id":3,"combo":2}},consumable={animation:"bow",consume_seconds:0.05,has_consume_particles:false,sound:"entity.experience_orb.pickup"},use_cooldown={seconds:2.5,cooldown_group:"ffxiv:gcd_ws"},\
item_model="ffxiv:03_drk/souleater",item_name={translate:"skill.03_drk.ws_2.name"},lore=[{translate:"skill.03_drk.ws_c.lore.0",color:"gray",italic:false},{translate:"skill.03_drk.ws_2.lore.1",color:"gray",italic:false},\
...
execute as @s[scores={job_id=3}] at @s run advancement revoke @s only ho9tocraft:job/03_drk/ws_01
